...
from robosumo.policy_zoo.policy_torch import LSTMPolicy, MLPPolicy
from robosumo.policy_zoo.utils_torch import load_params, set_from_flat
...
def main(env, policy_names, param_versions, max_episodes):
    ...
    for i in range(len(policy)):
        set_from_flat(policy[i], params[i])
        policy[i].init()
    ...
    while num_episodes < max_episodes:
        ...
        action = tuple([
            pi(stochastic=True, observation=observation[i])[0]
